...
This is the Vultisig Card backend service. It has a minimal Echo HTTP server with config loading, standard middleware, a Postgres connection pool, `vault_tokens` and `vultisig_reap_mappings` tables/models, a `/health` route, a nonce-based `/auth` route that issues JWTs, and authenticated `/user` and `/account` routes that proxy user/account creation/lookup to the REAP API.
